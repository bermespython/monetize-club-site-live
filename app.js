...
(function () {
  ...
  function openAuthModal(message) {
    ...
    setAuthMessage(message || '', !hasAuthConfig && !!message);
  }
  ...
  function handleSession() {
    if (!supabase) { applyAuthState(null); return Promise.resolve(); }
    return supabase.auth.getSession().then(function (res) {
      applyAuthState(res && res.data && res.data.session ? res.data.session.user : null);
    }).catch(function (err) {
      setAuthMessage(err && err.message ? err.message : String(err), true);
      applyAuthState(null);
    });
  }
